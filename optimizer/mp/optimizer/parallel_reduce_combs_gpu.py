...
def load_train_tensors(
    device: torch.device,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, np.ndarray, list[str]]:
    """
    ...
    target_cols = {"peak_down", "peak_up", "hemi_peak_down", "last_peak_type"}
    feature_cols = [
    ...
    return data_bool, peak_down, peak_up, hemi_peak_down, last_peak_type, feature_cols
...
def evaluate_batch(
    ...
    if profit_signal_mode == 1:
        ...
        # last_peak_type == 'down' AND hemi_peak_down == True
        is_down_mask  = torch.tensor(last_peak_type == "down", dtype=torch.float32, device=device)
        profit_signal = is_down_mask * hemi_peak_down                      # (N,)  float32
...
def grade_combs_gpu(
    ...
    data_bool, peak_down, peak_up, hemi_peak_down, last_peak_type, feature_cols = load_train_tensors(device)
    ...
    for batch_start in range(0, total, batch_size):
        ...
        results.extend(evaluate_batch(
            data_bool, peak_down, peak_up, hemi_peak_down, last_peak_type,
            col_index, batch,
